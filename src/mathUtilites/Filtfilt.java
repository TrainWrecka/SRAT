...
//Source: https://github.com/hdw09/filtfilt/blob/master/filtfilt-java/src/com/iiphci/dsp/Filtfilt.java
...
public class Filtfilt {
	...
	public static void filter(ArrayList<Double> B, ArrayList<Double> A, ArrayList<Double> X, ArrayList<Double> Y,
			ArrayList<Double> Zi) {
		if (A.size() == 0) throw new RuntimeException("A Êý×éÎª¿Õ£¡");
		boolean flagA = true;
	...
	static void add_index_range(ArrayList<Integer> indices, int beg, int end, int inc) {
	...
	static ArrayList<Double> subvector_reverse(ArrayList<Double> vec, int idx_end, int idx_start) {
		ArrayList<Double> resultArrayList = new ArrayList<Double>(idx_end - idx_start + 1);
